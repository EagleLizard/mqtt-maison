...
import { maison_actions } from '../../lib/models/maison-actions';
...
export async function mqttEzdMain() {
  ...
  let maisonCtrl = MaisonCtrl.init({
...
async function ikeaMsgHandler(ctx: MqttCtx, evt: MqttMsgEvt) {
  ...
  ctx.logger.debug(`action '${payloadStr}'`);
  ...
  await pubPromise;
  ctx.logger.info(`ikea adapter '${payloadStr}' -> '${mappedAction}'`);
}
